...
AgiborNet::AgiborNet() {
    ...
    this->send_ip.sin_port = htons(0); // let the system choose a random port
...
unsigned int CalculateCheckSum(const void* t, int size) {
    unsigned int sum = 0;
...
void AgiborNet::Send(int type,int id, float x, float y, float z, float w)
{
    ...
    packet.z = a_htonf(z);
    packet.w = a_htonf(w);
    packet.checksum = htonl(CalculateCheckSum(&packet, sizeof(packet)) + 0x464);
